...
class Beam():
    ...
    def __init__(self, bird:"Bird"):
        ...
        self.rct = self.img.get_rect() #ビームsurfaceのRectを抽出する
...
class Score:
    ...
    def __init__(self):
        ...
        self.rct = self.img.get_rect()
        self.rct.center = 100, HEIGHT - 50 #スコアを表示する場所
    def update(self, screen: pg.Surface):
        """
        スコアをアップデートして画面に表示する
        """
...
class Explosion:
    ...
    def update(self, screen: pg.Surface):
        ...
        """
        if self.life > 0:
            img = self.imgs[self.life % 2]  # 爆発画像を交互に切り替える
        ...
        for beam in beams[:]:
            for bomb in bombs[:]:
                if beam.rct.colliderect(bomb.rct):
                    score.add_score(1) #スコア加算
                    bombs.remove(bomb) #爆弾を削除
                    beams.remove(beam) #ビームを削除
                    break #一つのビームで複数爆を破壊しないようにする
        #画面外のビームを削除
